...
class WordPressRedTeam:
    ...
    def check_wordpress(self):
        ...
        try:
            ...
            markers = ['wp-content', 'wp-includes', 'wp-json', 'WordPress', 'xmlrpc', 'wp-login']
            found = any(marker in response.text for marker in markers)
            
            # Also check for WordPress in headers
            if 'X-Powered-By' in response.headers and 'WordPress' in response.headers.get('X-Powered-By', ''):
                found = True
            
            # If not found in text, try to access wp-login.php
            if not found:
                try:
                    login_response = self.session.get(self.target_url + '/wp-login.php', headers=self.headers, verify=False, timeout=5)
                    if login_response.status_code in [200, 403, 302]:
                        found = True
                except:
                    pass
...
def main():
    ...
    if not url.startswith(('http://', 'https://')):
        url = 'https://' + url
        print(f"{Fore.YELLOW}[!] Added https:// scheme: {url}{Style.RESET_ALL}")
